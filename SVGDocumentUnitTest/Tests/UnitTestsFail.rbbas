	...
	#tag Method, Flags = &h21
		Private Sub PassIfFailed()
		  If FailCount = ExpectedFailCount Then
		    ...
		    Else
		      Assert.Pass(Str(FailCount) + " tests have failed, as expected")
		    End If
		  Else
		    If ExpectedFailCount = 1 Then
		      Assert.Fail("Expected one failure, but had " + Str(FailCount))
		    Else
		      Assert.Fail("Expected " + Str(ExpectedFailCount) + " failures, but had " + Str(FailCount))
		    End If
		  End If
